...
struct bochscpu_hooks_t
{
    ...
    void (*opcode)(void*, uint32_t, void*, uint8_t*, uintptr_t, bool, bool) {};
...
extern "C"
{
    ...
    /// ignored.
    bochscpu_cpu_t
    bochscpu_cpu_new(uint32_t id);

    /// Create a new Cpu
    ///
    /// Instantiate an already existing cpu with the specified id.
    bochscpu_cpu_t
    bochscpu_cpu_from(uint32_t id);

    void
    bochscpu_cpu_forget(bochscpu_cpu_t p);

    /// Delete a cpu
    void
    bochscpu_cpu_delete(bochscpu_cpu_t p);

    void
    bochscpu_cpu_set_mode(bochscpu_cpu_t p);
    ...
    void
    bochscpu_cpu_run(bochscpu_cpu_t p, bochscpu_hooks_t** h);

    /// Stop emulation
    ///
    void
    bochscpu_cpu_stop(bochscpu_cpu_t p);

    void
    bochscpu_cpu_state(bochscpu_cpu_t p, bochscpu_cpu_state_t* s);

    void
    bochscpu_cpu_set_state(bochscpu_cpu_t p, const bochscpu_cpu_state_t* s);

    void
    bochscpu_cpu_set_state_no_flush(bochscpu_cpu_t p, const bochscpu_cpu_state_t* s);

    void
    bochscpu_cpu_set_exception(bochscpu_cpu_t p, uint32_t vector, uint16_t error);

    uint64_t
    bochscpu_cpu_rax(bochscpu_cpu_t p);

    void
    bochscpu_cpu_set_rax(bochscpu_cpu_t p, uint64_t val);

    uint64_t
    bochscpu_cpu_rcx(bochscpu_cpu_t p);

    void
    bochscpu_cpu_set_rcx(bochscpu_cpu_t p, uint64_t val);

    uint64_t
    bochscpu_cpu_rdx(bochscpu_cpu_t p);

    void
    bochscpu_cpu_set_rdx(bochscpu_cpu_t p, uint64_t val);

    uint64_t
    bochscpu_cpu_rbx(bochscpu_cpu_t p);

    void
    bochscpu_cpu_set_rbx(bochscpu_cpu_t p, uint64_t val);

    uint64_t
    bochscpu_cpu_rsp(bochscpu_cpu_t p);

    void
    bochscpu_cpu_set_rsp(bochscpu_cpu_t p, uint64_t val);

    uint64_t
    bochscpu_cpu_rbp(bochscpu_cpu_t p);

    void
    bochscpu_cpu_set_rbp(bochscpu_cpu_t p, uint64_t val);

    uint64_t
    bochscpu_cpu_rsi(bochscpu_cpu_t p);

    void
    bochscpu_cpu_set_rsi(bochscpu_cpu_t p, uint64_t val);

    uint64_t
    bochscpu_cpu_rdi(bochscpu_cpu_t p);

    void
    bochscpu_cpu_set_rdi(bochscpu_cpu_t p, uint64_t val);

    uint64_t
    bochscpu_cpu_r8(bochscpu_cpu_t p);

    void
    bochscpu_cpu_set_r8(bochscpu_cpu_t p, uint64_t val);

    uint64_t
    bochscpu_cpu_r9(bochscpu_cpu_t p);

    void
    bochscpu_cpu_set_r9(bochscpu_cpu_t p, uint64_t val);

    uint64_t
    bochscpu_cpu_r10(bochscpu_cpu_t p);

    void
    bochscpu_cpu_set_r10(bochscpu_cpu_t p, uint64_t val);

    uint64_t
    bochscpu_cpu_r11(bochscpu_cpu_t p);

    void
    bochscpu_cpu_set_r11(bochscpu_cpu_t p, uint64_t val);

    uint64_t
    bochscpu_cpu_r12(bochscpu_cpu_t p);

    void
    bochscpu_cpu_set_r12(bochscpu_cpu_t p, uint64_t val);

    uint64_t
    bochscpu_cpu_r13(bochscpu_cpu_t p);

    void
    bochscpu_cpu_set_r13(bochscpu_cpu_t p, uint64_t val);

    uint64_t
    bochscpu_cpu_r14(bochscpu_cpu_t p);

    void
    bochscpu_cpu_set_r14(bochscpu_cpu_t p, uint64_t val);

    uint64_t
    bochscpu_cpu_r15(bochscpu_cpu_t p);

    void
    bochscpu_cpu_set_r15(bochscpu_cpu_t p, uint64_t val);

    uint64_t
    bochscpu_cpu_rip(bochscpu_cpu_t p);

    void
    bochscpu_cpu_set_rip(bochscpu_cpu_t p, uint64_t val);

    uint64_t
    bochscpu_cpu_rflags(bochscpu_cpu_t p);

    void
    bochscpu_cpu_set_rflags(bochscpu_cpu_t p, uint64_t val);

    void
    bochscpu_cpu_es(bochscpu_cpu_t p, bochscpu_cpu_seg_t* s);

    void
    bochscpu_cpu_set_es(bochscpu_cpu_t p, const bochscpu_cpu_seg_t* s);

    void
    bochscpu_cpu_cs(bochscpu_cpu_t p, bochscpu_cpu_seg_t* s);

    void
    bochscpu_cpu_set_cs(bochscpu_cpu_t p, const bochscpu_cpu_seg_t* s);

    void
    bochscpu_cpu_ss(bochscpu_cpu_t p, bochscpu_cpu_seg_t* s);

    void
    bochscpu_cpu_set_ss(bochscpu_cpu_t p, const bochscpu_cpu_seg_t* s);

    void
    bochscpu_cpu_ds(bochscpu_cpu_t p, bochscpu_cpu_seg_t* s);

    void
    bochscpu_cpu_set_ds(bochscpu_cpu_t p, const bochscpu_cpu_seg_t* s);

    void
    bochscpu_cpu_fs(bochscpu_cpu_t p, bochscpu_cpu_seg_t* s);

    void
    bochscpu_cpu_set_fs(bochscpu_cpu_t p, const bochscpu_cpu_seg_t* s);

    void
    bochscpu_cpu_gs(bochscpu_cpu_t p, bochscpu_cpu_seg_t* s);

    void
    bochscpu_cpu_set_gs(bochscpu_cpu_t p, const bochscpu_cpu_seg_t* s);

    void
    bochscpu_cpu_ldtr(bochscpu_cpu_t p, bochscpu_cpu_seg_t* s);

    void
    bochscpu_cpu_set_ldtr(bochscpu_cpu_t p, const bochscpu_cpu_seg_t* s);

    void
    bochscpu_cpu_tr(bochscpu_cpu_t p, bochscpu_cpu_seg_t* s);

    void
    bochscpu_cpu_set_tr(bochscpu_cpu_t p, const bochscpu_cpu_seg_t* s);

    void
    bochscpu_cpu_gdtr(bochscpu_cpu_t p, bochscpu_cpu_global_seg_t* s);

    void
    bochscpu_cpu_set_gdtr(bochscpu_cpu_t p, const bochscpu_cpu_global_seg_t* s);

    void
    bochscpu_cpu_idtr(bochscpu_cpu_t p, bochscpu_cpu_global_seg_t* s);

    void
    bochscpu_cpu_set_idtr(bochscpu_cpu_t p, const bochscpu_cpu_global_seg_t* s);

    uint64_t
    bochscpu_cpu_cr2(bochscpu_cpu_t p);

    void
    bochscpu_cpu_set_cr2(bochscpu_cpu_t p, uint64_t val);

    uint64_t
    bochscpu_cpu_cr3(bochscpu_cpu_t p);

    void
    bochscpu_cpu_set_cr3(bochscpu_cpu_t p, uint64_t val);

    void
    bochscpu_cpu_zmm(bochscpu_cpu_t p, uintptr_t idx, bochscpu_cpu_zmm_t* z);

    void
    bochscpu_cpu_set_zmm(bochscpu_cpu_t p, uintptr_t idx, const bochscpu_cpu_zmm_t* z);

    uint32_t
    bochscpu_instr_bx_opcode(bochscpu_instr_t p);

    uint16_t
    bochscpu_instr_imm16(bochscpu_instr_t p);

    uint32_t
    bochscpu_instr_imm32(bochscpu_instr_t p);

    uint64_t
    bochscpu_instr_imm64(bochscpu_instr_t p);
    ...
    void
    bochscpu_mem_page_insert(uint64_t gpa, uint8_t* hva);

    /// Delete GPA mapping
    ///
    /// If the GPA is not valid, this is a no-op.
    void
    bochscpu_mem_page_remove(uint64_t gpa);
    ...
    void
    bochscpu_mem_missing_page(void (*handler)(uint64_t gpa));
    ...
    /// # Example
    uint8_t*
    bochscpu_mem_phy_translate(uint64_t gpa);
    ...
    uint64_t
    bochscpu_mem_virt_translate(uint64_t cr3, uint64_t gva);
    ...
    /// will panic
    void
    bochscpu_mem_phy_read(uint64_t gpa, uint8_t* hva, uintptr_t sz);
    ...
    /// will panic
    void
    bochscpu_mem_phy_write(uint64_t gpa, const uint8_t* hva, uintptr_t sz);
    ...
    int32_t
    bochscpu_mem_virt_write(uint64_t cr3, uint64_t gva, const uint8_t* hva, uintptr_t sz);
    ...
    /// Zero on success, non-zero on failure
    int32_t
    bochscpu_mem_virt_read(uint64_t cr3, uint64_t gva, uint8_t* hva, uintptr_t sz);

    void
    bochscpu_log_set_level(uintptr_t level);
